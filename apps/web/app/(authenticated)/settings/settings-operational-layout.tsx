...
type SettingsOperationalLayoutProps = {
  actions?: Array<{ href: string; icon?: string; label: string; variant?: "primary" | "secondary" | "ghost" }>;
  children: ReactNode;
  description: string;
...
const SETTINGS_SECTIONS = [
  ...
  { href: "/settings/permissions", icon: "lock", key: "permissions", label: "Permissions" },
  { href: "/settings/integrations", icon: "power", key: "integrations", label: "Integrations" },
...
export function SettingsOperationalLayout({
  ...
  if (variant === "editor") {
    return (
      <OperationalWorkspace
        ...
        data-settings-editor-route={route}
      >
        {toolbar}
  ...
  return (
    <OperationalWorkspace data-settings-detail-route={route}>
      {toolbar}
            ...
            {previewRows.map((row) => (
              ...
              >
                ...
                <span className="text-right font-semibold text-[var(--forge-text)]">
                  {row.tone ? <ForgeChip tone={row.tone}>{row.value}</ForgeChip> : row.value}
                </span>
              </div>
            ))}
          </div>
