...
export async function POST() {
  ...
  const result = await requestGoogleMeetSync(repository, authUser.id);
  ...
  if (result.ok) {
    ...
    await auditPlatformWorkspaceMutation(platformRepository, context, {
      action: "platform.workspace.google_meet.sync.request",
      metadata: { route: "/api/integrations/google-meet/sync" },
